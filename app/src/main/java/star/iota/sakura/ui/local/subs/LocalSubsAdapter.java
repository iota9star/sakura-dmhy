...
class LocalSubsAdapter extends RecyclerView.Adapter<LocalSubsAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(final MyViewHolder holder, int pos) {
        ...
        if (bean.getSub() != null) {
            holder.mButtonSub.setOnClickListener(view -> ((BaseActivity) holder.context).addFragment(
                    PostFragment.newInstance(bean.getSub().getUrl() + "/page/",
                            "",
                            bean.getSub().getName())));
        } else {
            holder.mButtonSub.setOnClickListener(view -> MessageBar.create(holder.context, "該發佈者無鏈接"));
        }
        holder.mButtonMagnet.setOnClickListener(view -> SimpleUtils.copy(holder.context, bean.getMagnet()));
        holder.mButtonLink.setOnClickListener(view -> SimpleUtils.openUrl(holder.context, bean.getUrl()));
        holder.mButtonDelete.setOnClickListener(view -> Observable.just(new SubsDAOImpl(holder.context))
                .map(subsDAO -> subsDAO.delete(bean.getId()))
                ...
                }, throwable -> MessageBar.create(holder.context, "刪除過程可能出現錯誤：" + throwable.getMessage())));
    }
